...
(function () {
	...
	var atomClicked = function (dataPoint) {
		var info = "[Node]={Id=" + dataPoint.id + "},{Symbol=" + dataPoint.symbol + "},{Size=" + dataPoint.size + "}";
	...
	var atomMouseover = function (dataPoint) {
		...
		var infoHtml = "Id=" + dataPoint.id + "<br/>Symbol="+dataPoint.symbol + "<br/>Size=" + dataPoint.size;

		divToolTip.transition()		
        	.duration(200)		
        	.style("opacity", 50);
		
		divToolTip.html(infoHtml);	
        	//.style("left", (d3.event.pageX) + "px")		
        	//.style("top", (d3.event.pageY - 30) + "px");
		
	};

	var atomMouseout = function (dataPoint) {
		divToolTip.transition()		
	        .duration(500)		
	        .style("opacity", 0);	
	};
	...
	var bondClicked = function (dataPoint) {
		var info = "[Bond]={Id=" + dataPoint.id + "},{Type=" + dataPoint.bondType + "}";
	 	if (bondSelected)
  ...
  var getRandomInt = function (min, max) {
	...
	$.getJSON(jsonUrl, function(json) {
		...
		newMoleculeSimulation (moleculeExamples, 'viewSet');
	});
	...
	var orgoShmorgo = function(graph) {
	  ...
	  /*
	   * ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
	   * force 레이아웃에서 핵심 코딩 부분 
	   * 
	   * ★ bondType : 숫자가 클수록 거리가 멀어짐 
	   * ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
	   */
	  ...
	  var force = d3.layout.force()
					...
					.linkDistance(function(d) { return radius(d.source.size) + radius(d.target.size) + 20 + (d.bondType * 1); })
					.on("tick", tick);
	  ...
	  function buildMolecule () {
		  ...
		  link.enter().insert("g", ".node")
		      ...
		      .each(function(d) {
		      	...
		      	d3.select(this)
		      		.append("line")
		      			.style("stroke-width", function(d) { return (1 * 3 - 2) * 2 + "px"; });
							/* .style("stroke-width", function(d) { return (d.bondType * 3 - 2) * 2 + "px"; }); 샘플소스(주석처리) */
			      ...
			      d3.select(this)
			      	.append("text")
						...
						d3.select(this)
							...
							.on("mouseover", atomMouseover)
							.on("mouseout", atomMouseout);
